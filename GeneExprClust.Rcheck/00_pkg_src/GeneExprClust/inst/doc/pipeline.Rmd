---
title: "Automated Gene Expression Analysis Pipeline: Real-World Application"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Automated Gene Expression Analysis Pipeline: Real-World Application}
...
This vignette demonstrates the power of the automated `run_pipeline()` function using a real-world gene expression dataset. Unlike the manual step-by-step approach shown in the first vignette, this automated pipeline provides a comprehensive analysis with a single function call, making it ideal for:

- **Quick exploratory analysis** of new datasets
- **Standardized workflows** across multiple projects  
- **Reproducible research** with consistent parameters
- **Batch processing** of multiple datasets

We'll analyze a time-course gene expression dataset from a cancer study, demonstrating how different clustering methods can reveal distinct temporal patterns of gene regulation.

# Dataset Description

We'll use the `ALL` (Acute Lymphoblastic Leukemia) dataset from Bioconductor, which contains gene expression profiles from 128 different individuals with B-cell acute lymphoblastic leukemia. This is a well-characterized dataset that's perfect for demonstrating clustering analysis.

**Key features:**
- **12,625 genes** measured using Affymetrix HG-U95Av2 chips
- **128 samples** from patients with different subtypes of ALL
- **Rich phenotype data** including age, sex, treatment response, and molecular subtypes
- **Clinical relevance** for understanding leukemia subtypes
...
For demonstration purposes, we'll focus on a subset of the data to make the analysis more manageable and interpretable.
...
Now we'll demonstrate the power of the automated pipeline. With a single function call, we'll perform comprehensive analysis including normalization, clustering, validation, visualization, and pathway enrichment.

## Basic Pipeline Run

Let's start with a basic analysis using default parameters:

```{r basic_pipeline, results='hide'}
...
Let's examine what the pipeline discovered:
...
## Advanced Pipeline Run with Multiple Methods

Now let's run a more comprehensive analysis comparing different clustering approaches:

```{r advanced_pipeline, results='hide'}
...
Let's examine how well the clustering captures known biological subtypes:

```{r biological_interpretation}
...
One of the most powerful features is the automated pathway enrichment analysis:
...
The pipeline automatically generates comprehensive visualizations. Let's examine some key plots:
